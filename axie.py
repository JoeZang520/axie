...
def loading(image_names, check_interval: float = 1, threshold=0.8, click_times=1, timeout=50, return_all_positions=False):
...
def detect_cards(color=True, quick_check=False):
    time.sleep(2)
    # 等待icon图片出现
    icon_result = loading(['icon'], click_times=0, timeout=15, return_all_positions=True)
...
def surrender(n):
    for i in range(n):
        ...
        loading(['icon'], check_interval=3, timeout=15)
